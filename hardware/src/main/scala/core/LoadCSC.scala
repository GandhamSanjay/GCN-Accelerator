 ...
 */
class LoadCSC(debug: Boolean = false)(implicit p: Parameters) extends Module with ISAConstants{
  ...
  val dec = Module(new LoadDecodeCSC)
  ...
  val scratchSel = Cat(dec.io.isPsum, dec.io.isRow, dec.io.isPtr, !dec.io.isSeq, dec.io.isVal) // row,ptr,den,val
