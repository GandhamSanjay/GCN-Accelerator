...
psumBin = dataGen.arrayToBinary(psum)
rowAddr = pow(2,10)
colAddr = 2 * pow(2,10)
valAddr = 3 * pow(2,10)
denAddr = 4 * pow(2,10)
...
instr = instr + instGen.spMM(sram_offset_col = colAddr, sram_offset_ptr = rowAddr, sram_offset_den = denAddr, sram_offset_val = valAddr, den_size = den.size, col_size = col.size, row_size = row.size, psum_valid = 1)
