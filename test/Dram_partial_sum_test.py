...
sparse = csr_matrix((val,col,row)).todense().astype(np.int64)
print(sparse)
O = np.matmul(sparse,den)
psum = np.ones((y,x)).astype(np.int64)
psum1 = (psum+psum).astype(np.int64)
sumBin = dataGen.matrixToBinary(psum)
sum1Bin = dataGen.matrixToBinary(psum1)
np.savetxt('output_matrix_0.txt',psum+O)
np.savetxt('output_matrix_1.txt',psum1+O)
...
instr = instr + instGen.load(xsize = den.size, id = 'den', dram_offset = denAddr, sram_offset = 0, final_load = 0, denGroup = 1)
instr = instr + instGen.load(xsize = psum.size, id = 'psum', dram_offset = sumAddr, sram_offset = 0, final_load = 0, denGroup = 0)
instr = instr + instGen.load(xsize = psum1.size, id = 'psum', dram_offset = sum1Addr, sram_offset = 0, final_load = 1, denGroup = 1)
...
instr = instr + instGen.spMM(sram_offset_col = colAddr, sram_offset_ptr = rowAddr, sram_offset_den = denAddr, sram_offset_val = valAddr, den_size = den.size, col_size = col.size, row_size = row.size, pr_valid = 1, sram_offset_partial_sum = sumAddr, add_partial_sum = 1, scratchpad_n_global_buffer = 0, nnz_per_group = int(col.size/nPEs), dense_loaded = 1)
